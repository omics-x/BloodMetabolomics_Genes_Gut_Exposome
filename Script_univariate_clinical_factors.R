######################################################################### 
### Script for univariate association analysis of clinical factors with metabolomics 
#########################################################################
...
######################################################################### 
### Regression analysis fpr association analysis of clinical factors with metabolomics 
#########################################################################
library(dplyr)
library("readxl")

clinical<-read.csv("comorbidity_jun_2022.csv")
metabolom<-read.csv("metabolomics_data_comorbidity_jun_2022.csv")
colnames(clinical)[1]<-"ergoid"
colnames(metabolom)[1]<-"ergoid"
merge<-merge(metabolom,clinical,by.x='ergoid',by.y='ergoid',x.all=T)
results <- data.frame(
...
demo_list<-colnames(clinical)[c(2:5)]
...
write.csv(res_anno, file="Association_analysis_clinical.csv")
