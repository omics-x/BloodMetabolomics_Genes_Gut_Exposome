### Script for association of metabolites with general cognition. 
library(foreign)
library(dplyr)
library("readxl")
...
### merging the data the cognition information file with covariate information and metabolomics data
...
### FAC1_1 is coding for the general cognition (g-factor) phenotype file
endo_pheno<-c("FAC1_1")
...
merg2$g_factor<-merg2[,endo_pheno[j]]
 test<- summary(lm (paste("g_factor ~ metabo + Age_blood_collection + sex + Lipilower + BMI",sep=""), data=merg2))
#test<- summary(lm (paste("g_factor ~ metabo + Age_blood_collection + sex + Lipilower + BMI + education",sep=""), data=merg2))
tablerow <- data.frame(
            ...
            n=dim(na.omit(merg2[,c("metabo","g_factor")]))[[1]],
            stringsAsFactors=FALSE)
...
anno_com <- read_excel("DUKE-0304-19ML+_CLIENT DATA TABLE.XLSX",sheet = 2)
...
# Repeat the association for both males and females separately and run below code accordinly before saving files 
...
merg2_m$g_factor<-merg2_m[,endo_pheno[j]]
test<- summary(lm (paste("g_factor ~ metabo + Age_blood_collection + Lipilower + BMI",sep=""), data=merg2_m))
tablerow <- data.frame(
            endo_pheno=endo_pheno[j],
            Metabolite=metabolites[i],
            Beta=test$coefficients[2,1],
            Se=test$coefficients[2,2],
            p=test$coefficients[2,4],
            n=dim(na.omit(merg2_m[,c("metabo","g_factor")]))[[1]],
